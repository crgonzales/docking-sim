...
- `flightGraphics` resolves query → guarded storage → Balanced; High is opt-in. Both cap scene allocation at 2.5M pixels and keep the medium cloud view at its existing 1.5M ceiling. Balanced requests DPR1, medium SMAA, 2048 shadows and 8x anisotropy; High requests DPR1.5, high SMAA, 4096 shadows and 16x anisotropy, hardware bounded. The local shadow footprint remains 110m for walking/chase coverage, with a bounded texel-scaled normal offset. Only the imported hull casts from its back faces to avoid skin self-shadow bands; thin parts retain their original casting. Paused switching preserves pose/time and the cloud owner. The frozen cloud-base fixture ignores stored preferences; a valid graphics query or deliberate paused panel choice opts into the new presets.
