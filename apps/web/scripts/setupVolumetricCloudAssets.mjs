...
const outputDirectory = path.join(webDirectory, 'public', 'assets', 'clouds', 'volumetric');
...
async function main() {
  ...
  for (const [name, asset] of Object.entries(assets)) {
    ...
    manifestAssets[name] = {
      path: `/assets/clouds/volumetric/${asset.file}`,
  ...
  const manifest = {
    schemaVersion: 1,
    generatedBy: 'apps/web/scripts/setupVolumetricCloudAssets.mjs',
    deterministic: true,
    ...
    limitations: [
      ...
      'the opt-in VOLUMETRIC backend bypasses stock weather masks; default promotion awaits whole-renderer acceptance'
    ],
    assets: manifestAssets
  };
  await writeFile(path.join(outputDirectory, 'manifest.json'), `${JSON.stringify(manifest, null, 2)}\n`);
  process.stdout.write(`Generated VOLUMETRIC cloud assets in ${path.relative(webDirectory, outputDirectory)}\n`);
