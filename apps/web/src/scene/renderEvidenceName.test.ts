...
describe('development evidence recorder names', () => {
  it('fits the existing recorder whitelist and 100-character limit for every supported mode', () => {
    for (const backend of ['eve', 'lib', 'old'] as const) {
  ...
  it('keeps repeated and differently staged captures distinct', () => {
    expect(new Set([
      renderEvidenceName('eve', 'medium', 'albedo', 1),
      renderEvidenceName('eve', 'medium', 'lighting', 1),
      renderEvidenceName('eve', 'medium', 'full', 1),
      renderEvidenceName('eve', 'medium', 'full', 2),
    ]).size).toBe(4);
  });
  it('does not include arbitrary query content in a filesystem name', () => {
    expect(renderEvidenceName('eve', 'medium', '../'.repeat(100), 1)).toBe('capture-eve-medium-full-1');
  });
});
