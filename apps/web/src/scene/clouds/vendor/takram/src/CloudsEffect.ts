...
export interface CloudsEffectOptions {
  ...
  /** Shared cloud lighting replaces the unused Beer-map producer. */
...
export class CloudsEffect extends Effect {
  ...
  private readonly sharedLighting: boolean
  ...
  override initialize(
    ...
    if (!this.sharedLighting) this.shadowPass.initialize(renderer, alpha, frameBufferType)
  ...
  private updateSharedUniforms(deltaTime: number): void {
    ...
    const ecefToWorldRotation = rotationScratch
      ...
      .transpose()
    if (!this.sharedLighting) this.shadowMaps.update(
  ...
  private updateAtmosphereComposition(): void {
    ...
    // Shared consumers query the same light volume; no Beer shadow is published.
    if (this.sharedLighting) return
